...
- (NSArray *)get_all_album_cover
{
    ...
    [self.queue inDatabase:^(FMDatabase *adb) {
        ...
        int count = 0;
        FMResultSet *rsl = [adb executeQuery:@"select url from image_infos where face_image = 1"];
        while ([rsl next]) {
            count++;
       
            NSString *url               = [[[rsl stringForColumn:@"url"] base64DecodedString] stringByReplacingOccurrencesOfString:[NSString stringWithFormat:@"_%@",base64_seed] withString:@""];
